@if(isset($saveAction['active']) && !is_null($saveAction['active']['value']))
    <div id="saveActions" class="form-group">
        ...
        @if(!empty($saveAction['options']))
            <div class="btn-group" role="group">
                ...
                <button type="submit" class="btn btn-success">
                    ...
                    <span data-value="{{ $saveAction['active']['value'] }}">Hello</span>
                </button>
        ...
        @if(!$crud->hasOperationSetting('showCancelButton') || $crud->getOperationSetting('showCancelButton') == true)
            <a href="{{ $crud->hasAccess('list') ? url($crud->route) : url()->previous() }}" class="btn btn-default"><span class="la la-ban">Test button</span> &nbsp;{{ trans('backpack::crud.cancel') }}</a>
        @endif
